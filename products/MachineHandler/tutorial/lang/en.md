...
permalink: /products/MachineHandler/tutorial/en
...
MachineHandler is a program intended to be used as submodule of [CheckDoseQt](http://PotatoBite.github.io/products/CheckDoseQt). Its main function is to provide Clinical Physicist with a tool for editing the clinic database used by CheckDoseQt and modify the values that associated Linacs and Beams hold.


###  Creating a Clinic ###

First of all, before using CheckDoseQt, you have to provide through MachineHandler a valid Clinic Database file containing the info about the Linacs and their associated beams you will use in CheckDoseQt.

Naturally, the first task is to create a clinic. A clinic is nothing but an encrypted sqlite DB file containing in tables all the info required for CheckDoseQt to properly work. The database is generated automatically by the MachineHandler module when created. Therefore, must be mandatory avoided to edit it manually.

In the Clinic Info dialog select `Add Clinic` button to create a new clinic:

!["Adding a Clinic"](./res/add_clinic.png)

When creating a clinic, an encrypted sqlite file is created automatically and there is not need of man intervention in the process. All required tables, default values and templates are automatically added executing a series of SQL statements sequence, reason which can lead to delays in the process, mainly in old systems.
...
MachineHandler provides a selector of valid Clinic Database files through a combo box selector Widget. Changing the selected widget value updates the Linacs' Table in the downside of the Dialog. 

The Linacs' table shows the Linacs defined in the current selected clinic, their ID, Name and State. If no Linac has been defined yet,  the table is shown empty.

When creating a new Clinic, the Clinic Selector updates its present internal list of clinic files at the Runtime directory.
...
When the `Edit Clinic` button is clicked, a Dialog asking a new name for your clinic is shown. Providing  a new name for your clinic and accepting the dialog makes that the selected clinic changes its name. 
...
### Adding Linacs to the Clinic ###

Adding a Linac is as simple as clicking the `Add Linac` button after selecting the clinic at which you want to store your Linac with the `Clinic Selector` combo box. A dialog containing mandatory fields to be fill by user is shown
. 
...
| :--------------- | :----------------------------------------------------------- | --------------------------------------------------- |
| Machine Name     | 4-12 alphanumeric characters                                 | Name shown for the current Linac in DB             |
...
After filling all fields, click the `Next` button to start defining **Beams** for your **Linac**.
...
The Beam Info Dialog contains an Energy selector and Energy type selector. Additionally, a Beam table is shown in the downside, similar to the one that is shown in the Clinic Info Dialog. The table shows the Beams that belongs to the current selected Linac. If no Beam has been created yet, is normal that the tables stays in an empty state.

After the Linac has been created, at least Beam must be added and validated before can be used in CheckDoseQt. 
...
A dialog asking for mandatory information must be shown:

![](./res/beam_commissioning.png)

Mandatory fields must be filled in order to accept and continue to the next stage. For completing this task, values introduced in the present and forward dialog must be [AAPM TG-114](https://aapm.onlinelibrary.wiley.com/doi/abs/10.1118/1.3521473). compliant. Some of them are shown here for reference only, for more information about the definition of these parameters, please, refer to the [AAPM TG-114](https://aapm.onlinelibrary.wiley.com/doi/abs/10.1118/1.3521473).
...
| Ref. Field Size | The reference field size (r_0) for the treatment modality that defines the reference dose rate D_0_rate |

After filling the parameters, click `Next` to start providing the Beam Descriptor files in the following Dialog.
...
The next dialog is intended to be used for directly import into database the files that contains the data of each beam. Once the beam is created, you will need to import the Beam's descriptor parameters files. 
...
The `C` button aside each label clears the value of the selected file, in case you confuse and select a wrong file in a different category or similar.

Once you have selected all files you want to import, clicking the `Validate` button will trigger a series of revision of given files in order to detect anomalies or irregularities. Once all tests are finished, you will see some Dialogs giving information about the result of the checking process, and asking if you want to come back to (maybe) re-enter some files.

!["BeamDescriptor tests"](./res/beam_descriptor2.png)

If the file passes the test, its label is shown in dark green background, if it does not pass the test, it will be shown in a red background color, like in the previous picture.
...
Once you have validated all the files needed by the Machine you can close the Dialog accepting the prompts shown after clicking the `Validate` button.
...
Contact us at PotatoBite@gmail.com if any doubt or inquery.
